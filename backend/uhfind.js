...
function fetchCourses( campus, dept, callback ) {
  ...
  var url = 'https://www.sis.hawaii.edu/uhdad/avail.classes?i=' + campus + 
            '&t=201530&s=' + dept;
...
/* scrapeDom 
Private helper function that performs the actual string parsing required by 
fetchCourses(). 

This function runs in the context of the webpage that was given to it in 
fetchCourses() - almost as if it were running in a browser javascript console.
It directly accesses dom elements on the page and returns the course data as an
...
function scrapeDom() {
  ...
  for (var i = 2; i < rows.length; i++) {
    ...
    if ( ! isSubstring(rows[i].className, 'section-comment-course')) { 
      ...
      in a smart way I am simply hardcoding the column indices and commenting 
      out my old code.  I will fix this eventually but honestly can't be 
      bothered.
      **/
